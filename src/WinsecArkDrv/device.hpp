...
#include "logger.hpp"
namespace winsec::device
{
	constexpr int io_control_code = CTL_CODE(FILE_DEVICE_UNKNOWN, 0x830, METHOD_BUFFERED, FILE_ANY_ACCESS);

    inline UNICODE_STRING g_device_name = RTL_CONSTANT_STRING(L"\\Device\\WinSec-ARK");
    inline UNICODE_STRING g_symbol_link_name = RTL_CONSTANT_STRING(L"\\??\\WinSec-ARK");
...
inline NTSTATUS winsec::device::init_device(PDRIVER_OBJECT driver_object)
{
    ...
    do
    {
		...
		{
			winsec::logger::log_debug("Failed to create device object (0x%08X)\n", status);
			break;
		}
		...
		if (!NT_SUCCESS(status))
		{
			winsec::logger::log_debug("Failed to create symbol link (0x%08X)\n", status);
			break;
		}
		...
		if (!NT_SUCCESS(status))
		{
			winsec::logger::log_debug("Failed to register shutdown notification  (0x%08X)\n", status);
			break;
		}
...
NTSTATUS winsec::device::un_init_device()
{
	NTSTATUS status = STATUS_SUCCESS;
...
NTSTATUS winsec::device::register_shutdown_notification(PDEVICE_OBJECT device_object)
{
	NTSTATUS status = STATUS_SUCCESS;
